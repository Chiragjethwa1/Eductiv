...
require("E:/STUDY/xampp/htdocs/Eductiv/EductivDatabase.php");  // attach new latest file
...
class EductivAdministrator extends EductivDatabase
{
    ...
    {
        $result = EductivAdministrator::query(
            "SELECT `User` FROM `mysql`.`user` WHERE `User` = BINARY ? AND `Password` = BINARY ?;",
            'ss',
            array($this->admin_data["username"], $this->admin_data["username"]),
            true
        );

        $admin = new mysqli("localhost", $this->admin_data["username"], $this->admin_data["password"], "eductiv");
        ...
        /*if ($admin) {
            EductivAdministrator::writeLog("ADMIN", "info", $this->admin_data["username"] . " logged in.");
            return true;
            //return EductivAdministrator::putResponse("success", "message", "Logged in.");
        } else {
            EductivAdministrator::writeLog("ADMIN", "error", "Log in failed [USER: " . $this->admin_data["username"] . " PASSWORD: " . $this->admin_data['password'] . "]");
            return EductivAdministrator::putResponse("error", "message", "Log in attempt failed!");
        }*/
    }
}
